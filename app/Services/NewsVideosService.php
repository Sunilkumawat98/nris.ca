...
class NewsVideosService
{
    ...
    **/
    ...
    public function fetchHomePageNews($param)
    {
        ...
        $result                                     = NewsVideo::where('is_live',1)
                                                        ->where('status',1)
                                                        ->take(4)
                                                        ->orderBy('id', 'DESC');
        ...
        if(count($result)>0)
        {
            $result->makeHidden(['meta_title', 'meta_description', 'meta_keywords']);
    ...
    **/
    ...
    public function fetchAllNewsById($param)
    {
        $response = [
            $this->status => false,
        ...
        $result = NewsVideo::where('id', $param['news_id'])
            ->where('is_live', 1)
            ->where('status', 1)
            ->orderBy('id', 'DESC')
            ->first();
        ...
        if ($result) {
            ...
            $suggestions = NewsVideo::where('is_live', 1)
                    ->where('status', 1)
                    ->where('id', '!=', $param['news_id'])  // Exclude the news_id
                    ->take(8)
                    ->orderBy('id', 'DESC')
                    ->get();
            ...
            $response[$this->message] = 'Successfully data found..';
            $response[$this->code] = 200;
            $response[$this->data] = $result;
            $response[$this->data]['suggestions'] = $suggestions;
            

        } else {
            $response[$this->status] = false;
            $response[$this->message] = 'Data not found...';
