...
class MovieVideoCategoryController 
{
    public function __construct()
    {
        ...
        $this->permission               = 'movie_videos_category';
        $this->route                    = 'movie_video_category';
    ...
    {     
        ...
        $results                      = MovieVideoCategory::where('status', 1);
        if ($searchQuery) {
    ...
    public function show($id)
    {
        ...
        $results                        = MovieVideoCategory::findOrFail($id);
    ...
    public function update(Request $request, $id)
    {
        ...
        $result                         = MovieVideoCategory::findOrFail($id);
        $result->update($all);
    ...
    public function destroy(MovieVideoCategory $movieVideoCategory)
    {
        if(!auth()->user()->hasPermission('delete_'.$this->permission))
        {
            abort(404, 'You are not Authorised...');
        }
        $movieVideoCategory->delete();
